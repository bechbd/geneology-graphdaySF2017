...
public class Import {
        ...
        for (File file: files) {
            if (!file.getName().startsWith(".")) {
                ...
                for (Individual i : g.getIndividuals().values()) {
                    ...
                    cnt++;
                    if (cnt > 0 && cnt % 100 == 0) {
                        System.out.println("Writing out individual " + cnt);
                    }
                }
    ...
    public static DseSession ConnectToDSE() {
        ...
        try {
            cluster = DseCluster.builder()                                               // (1)
                    .addContactPoint("192.168.57.143")
                    .withGraphOptions(new GraphOptions().setGraphName("geneology"))
                    .build();
    ...
    public static void insertIndividual(DseSession session, UUID treeId, Individual individual) {

        UUID id = UUID.randomUUID();
        try {
            session.executeGraph("g.addV('individual')" +
                    ...
                    ImmutableMap.<String, Object>builder()
                            .put("unique_id", id)
                            .put("gedcom_id", individual.getXref().toString())
                            .put("full_name", individual.getFormattedName() != null ?
                                    individual.getFormattedName().toString() : "")
                            .put("sex", individual.getSex() != null ?
                                    individual.getSex().toString() : "")
                            .put("tree_id", treeId)
                            .build());
        } catch (Exception ex) {
            System.out.println("Individual: " + ex.toString() + individual.toString());
        }
        for (PersonalName n : individual.getNames()) {

            try {
                session.executeGraph("g.V().has('individual', 'unique_id', individual_id)" +
                ...
                if (c.getIndividual()!=null) {
                    try {
                        session.executeGraph("g.V()" +
                                        ...
                                        ".V().has('individual', 'tree_id', tree_id)" +
    ...
    private static void insertTreeNode(DseSession session, UUID treeId, String ownerXrefId) {
        try {
            session.executeGraph("g.addV('tree')" +
                            ...
                            ".as('t')" +
                            ".coalesce(__.V().has('individual', 'tree_id', tree_id)" +
                            ".has('individual', 'GEDCOM_xref_id', owner_id)," +
                            "__.addV('individual').property('tree_id', tree_id)" +
